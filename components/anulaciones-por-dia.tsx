...
import type { SolicitudAnulacion } from "@/lib/types";
...
export function AnulacionesPorDia({ semanaInicio, onPrev, onNext, onHoy }: Props) {
  const [items, setItems] = useState<SolicitudAnulacion[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const data = await apiRequest<SolicitudAnulacion[]>(
        `/api/v1/solicitudes-anulacion?${params.toString()}`,
      );
      setItems(data);
    } catch (err) {
      setError(err instanceof Error ? err.message : "No se pudo cargar el control de anulaciones");
    } finally {
      setLoading(false);
    }
  }, [semanaInicio]);
    ...
    <div className="flex flex-col gap-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <p className="m-0 text-sm font-semibold capitalize text-[var(--primary-color)]">
          Semana {etiquetaSemana(semanaInicio)} · {items.length} mandada
          {items.length === 1 ? "" : "s"} a anular
        </p>
      </div>
        ...
        {dias.map((dia) => (
          ...
          >
            <header className="flex items-center justify-between bg-[var(--primary-50,#f3eaf9)] px-4 py-2">
              ...
              <Tag value={`${dia.delDia.length}`} severity="danger" />
            </header>
            {dia.delDia.length === 0 ? (
              <p className="m-0 px-4 py-3 text-sm text-[var(--text-color-secondary)]">
                Ningún técnico mandó a anular este día
              </p>
            ) : (
              <ul className="m-0 list-none p-0">
                {dia.delDia.map((item) => {
                  const { nombre } = parseNombreCliente(item.cliente);
                  return (
                    <li
                      key={item.id}
                    ...
                    >
                      ...
                      <Tag value={formatOrdenNumero(item.numeroOrden)} severity="info" />
                      <span className="font-semibold">{titleCase(nombre || item.cliente)}</span>
                      <span>
                        {titleCase(item.colonia) || "Sin barrio"} · {titleCase(item.ciudad)}
                      </span>
                      <span>{formatTelefono(item.telefono)}</span>
                      <span className="text-[var(--text-color-secondary)]">{item.motivo}</span>
                      <span className="text-[var(--text-color-secondary)]">
                        App · {titleCase(item.solicitadoPor)}
                      </span>
                    </li>
                  );
                })}
